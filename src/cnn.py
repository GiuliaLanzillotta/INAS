...
class cnn():
    ...
    def build_child_arch(self, action):
        ...
        self.net = conv_net(state, input_size=self.original_image_size, prev_channels=self.prev_channels,
                            n_class=self.num_classes, device=self.device)
        self.net = self.net.to(device)
        # Updating and returning the new state
        self.state = state
        return self.state

    "This function is used to build the best CNN state into an architecture!"
    def bestCNN(self,state):
        self.original_image_size = 32
